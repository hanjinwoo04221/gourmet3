 ...
 * pressure, and the Appetite drain grows with it.
 */
public class KiReleaseSkill implements SkillBehavior {
    ...
    public int extraAppetiteCost(TorikoData data) {
        float fraction = CellEvolution.powerFraction(data.kiOutputSetting(), CellEvolution.KI_OUTPUT_BASE,
                CellEvolution.kiOutputCap(data.cellLevel()));
        return Math.round(fraction * MAX_EXTRA_COST);
    }
